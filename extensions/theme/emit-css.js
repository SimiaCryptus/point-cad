...
export function emitCSS(sketch, result = null, options = {}) {
  ...
  if (typeof sp.formatLiteral !== "function") throw new Error(`Space '${sketch.space}' cannot format colours`);
  ...
  const decls = (colors) => points.map((p) => {
    const c = colors.get(p.id);
    const name = `--${prefix}${kebab(p.label)}`;
    const lines = [];
    if (fallback && format !== "rgb" && format !== "hex") lines.push(`  ${name}: ${sp.formatLiteral(c, "rgb")};`);
  ...
  const out = [];
  if (header) out.push(`/* Point-CAD theme — ${points.length} colour${points.length === 1 ? "" : "s"}${themes[0].id ? `, themes: ${themes.map((t) => t.id).join(", ")}` : ""} */`);
  out.push(block(scope, def.colors));
  for (const t of themes) {
    if (t.id == null || t === def) continue;
    if (modes.has("media") && (t.id === "dark" || t.id === "light")) {
      out.push(`@media (prefers-color-scheme: ${t.id}) {\n${block(scope, t.colors).replace(/^/gm, "  ")}\n}`);
    }
  }
  for (const t of themes) {
