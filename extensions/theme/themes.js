...
export const POINT_ROLES = [
  { id: "anchor", fixed: true, export: true, description: "Theme input that is also a token (brand colour, canvas)" },
  { id: "orientation", fixed: true, export: false, description: "Theme input that steers derived colours but is not a token" },
...
export function implicitGamutConstraints(sketch, reg) {
  ...
  const covered = new Set(sketch.constraints.filter((c) => c.type === "gamut").flatMap((c) => c.points));
...
export function solveThemes(sketch, { registry: reg = defaultRegistry, scenarios = true, primary = null, ...solverOptions } = {}) {
  const solver = reg.solvers.has(sketch.solver.method) ? reg.getSolver(sketch.solver.method) : [...reg.solvers.values()][0];
  if (!solver) throw new Error("No solver registered");
  const problem = buildProblem(sketch, reg, { scenarios: sketch.scenarios?.length ? scenarios : undefined, primary });
  return solver.solve(problem, { maxIterations: sketch.solver.maxIterations, tolerance: sketch.solver.tolerance, ...solverOptions });
}
...
export function scenarioColors(sketch, result = null, scenarioId = null) {
  ...
  for (const p of sketch.points) {
    const c = sol?.points?.[p.id] ?? scen?.points?.[p.id] ?? scen?.points?.[p.label] ?? p.solved ?? p.seed;
