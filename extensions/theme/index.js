...
export function registerTheme(reg = defaultRegistry) {
  ...
  for (const k of colorConstraintKinds)
    if (!reg.hasConstraintKind(k.id)) reg.registerConstraintKind(k);
  for (const st of [themeStatement, gamutStatement])
    if (!reg.hasStatement(st.id)) reg.registerStatement(st);
  for (const r of POINT_ROLES) if (!reg.pointRoles.has(r.id)) reg.registerPointRole(r);
  if (!reg.implicitConstraints.includes(implicitGamutConstraints))
