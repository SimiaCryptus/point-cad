...
export function createPreviewPanel(ctx, overrides = {}) {
  ...
  const themeSel = h('select', {
    ...
    onchange: () => ctx.setActiveScenario(themeSel.value || null),
  });
  const status = h('div', { class: 'pc-muted pc-preview-status' });
  const htmlTa = h('textarea', {
    class: 'pc-template pc-grow',
    spellcheck: 'false',
    oninput: () => themeDoc.patchPreview({ html: htmlTa.value }),
  });
  const cssTa = h('textarea', {
    class: 'pc-template pc-grow',
    spellcheck: 'false',
    oninput: () => themeDoc.patchPreview({ css: cssTa.value }),
  });
  ...
  const resetBtn = (fn) =>
    h(
      'button',
      { class: 'pc-btn', type: 'button', title: 'Restore the default template', onclick: fn },
      'Reset'
    );
  ...
  const panes = {
    ...
    html: h(
      ...
      h(
        'div',
        { class: 'pc-row' },
        copyBtn(() => htmlTa.value),
        resetBtn(() => themeDoc.patchPreview({ html: DEFAULT_PREVIEW_HTML }))
      )
    ),
    css: h(
      'div',
      ...
      h(
        ...
        { class: 'pc-row' },
        copyBtn(() => cssTa.value),
        resetBtn(() => themeDoc.patchPreview({ css: DEFAULT_PREVIEW_CSS }))
      )
    ),
  };
  const tabBtns = {};
