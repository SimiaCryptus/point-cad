...
const sameTarget = (a, b) => JSON.stringify([a?.kind, a?.value ?? null, a?.ref ?? null]) === JSON.stringify([b?.kind, b?.value ?? null, b?.ref ?? null]);
...
function targetText(c, kind, varIdent) {
  ...
  switch (t.kind) {
    case "value": case "variable": return `= ${rhs}`;
    case "atLeast": return `>= ${rhs}`;
    case "atMost": return `<= ${rhs}`;
...
export function emitWithMap(sketch, { adoptSolution = false, header = true, registry = null } = {}) {
  ...
  const space = registry && registry.hasSpace(sketch.space) ? registry.getSpace(sketch.space) : null;
  const vec = (v) => (space && typeof space.formatLiteral === "function" ? space.formatLiteral(v) : tuple(v));
  const kindNames = new Set(registry ? registry.constraintKindsFor(sketch.space).map((k) => k.id) : sketch.constraints.map((c) => c.type));
  const extraReserved = new Set([...kindNames, ...(registry ? [...registry.statements.keys(), ...registry.pointRoles.keys()] : [])]);
  const pointIdent = identMap(sketch.points, "label", extraReserved);
  const varIdent = identMap(sketch.variables, "name", extraReserved);
  const kindOf = (type) => (registry && registry.hasConstraintKind(type) ? registry.getConstraintKind(type) : null);
  ...
  const extLines = [];
  if (registry) for (const st of registry.statements.values()) if (typeof st.emit === "function") extLines.push(...(st.emit(sketch, helpers) ?? []));
  ...
  for (const v of vars) {
    ...
    add(`var ${varIdent.get(v.id)} = ${fmtNum(value)}${v.locked ? " locked" : ""}${v.shared ? " shared" : ""}`, v.id);
  }
  ...
  for (const c of constraints) {
    ...
    const parts = [c.type, ...paramNames.map((k) => String(c.params?.[k] ?? "")).filter(Boolean), ...c.points.map((id) => pointIdent.get(id) ?? id)];
  ...
  const scenarioWord = (registry && [...registry.statements.values()].find((s) => s.scenarioKeyword)?.id) ?? "scenario";
  ...
  add(`view camera ${tuple(cam.position)} target ${tuple(cam.target)} up ${tuple(cam.up)} ${cam.projection} fov ${fmtNum(cam.fov)}`, "view");
