...
export class PointCad extends HTMLElement {
  ...
  static registerPanel(name, factory, title = name) {
    ...
    if (typeof document !== 'undefined') {
      for (const el of document.querySelectorAll('point-cad'))
        if (el instanceof PointCad && el._built) el._buildPanels();
    }
  }
  ...
  connectedCallback() {
    ...
    if (this.hasAttribute('src') && this.getAttribute('src') !== this._loadedSrc)
  ...
  set activeScenario(id) {
    ...
    if (sol) {
      for (const p of this._sketch.points)
        if (sol.points[p.id]) p.solved = sol.points[p.id].slice();
      for (const v of this._sketch.variables)
  ...
  fromScript(text) {
    const base = createSketch({
      view: cloneDeep(this._sketch.view),
      solver: cloneDeep(this._sketch.solver),
    });
  ...
  solve(options = {}) {
    ...
    try {
      ...
      result = solver.solve(problem, {
        maxIterations: sk.solver.maxIterations,
        tolerance: sk.solver.tolerance,
        ...solverOptions,
      });
    } catch (e) {
  ...
  _adopt(res, source, reason) {
    ...
    for (const cmd of res.commands) {
      ...
      try {
        this.solve(
          cmd.scenario ? { scenarios: cmd.scenario } : cmd.scenarios ? { scenarios: true } : {}
        );
      } catch {
  ...
  _refresh() {
    ...
    this._projBtn.textContent =
      this._sketch.view.camera.projection === 'orthographic' ? 'Ortho' : 'Persp';
  ...
  _makeContext() {
    ...
    return {
      ...
      get activeScenario() {
        return self._activeScenario;
      },
      setActiveScenario: (id) => {
        self.activeScenario = id;
      },
  ...
  _build() {
    ...
    this._toolWindows = h('div', {
      class: 'pc-toolbar-group',
      role: 'toolbar',
    ...
    this._projBtn = tool('Persp', 'Toggle perspective / orthographic (5)', () =>
      vp.toggleProjection()
    );
    this._toolView = h(
      'div',
      { class: 'pc-toolbar-group', role: 'toolbar', 'aria-label': 'View' },
      tool('Solve', 'Solve the sketch', () => this._ctx.solve(), 'pc-primary'),
      tool('Center', 'Re-centre the sketch and align it to its principal axes', () =>
        this.center()
      ),
    ...
    root.addEventListener('keydown', (e) => {
      ...
      if (
        ...
        e.target === this._viewport.el &&
        this._selection.size &&
        !this.readonly
      ) {
  ...
  _buildPanels() {
    ...
    for (const name of wanted) {
      ...
      const w = {
        ...
        panel,
        index: index++,
        open: old ? old.open : initiallyOpen.has(name),
        x: old?.x ?? null,
        y: old?.y ?? null,
      };
      ...
      w.btn = h(
        'button',
        {
          class: 'pc-btn pc-tool pc-toggle',
          type: 'button',
          title: `Show / hide the ${TITLES[name] ?? name} window`,
          'aria-pressed': 'false',
