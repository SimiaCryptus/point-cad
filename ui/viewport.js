...
export class Viewport {
  ...
  ray(sx, sy, fr) {
    ...
    if (fr.projection === 'perspective') {
      const d = V.normalize(
        V.add(fr.f, V.add(V.scale(fr.r, px / fr.focal), V.scale(fr.u, py / fr.focal)))
      );
      return { o: fr.pos, d };
    }
  ...
  render() {
    ...
    for (const c of sk.constraints) {
      ...
      const kind = this.ctx.registry.hasConstraintKind(c.type)
        ? this.ctx.registry.getConstraintKind(c.type)
        : null;
    ...
    for (const p of sk.points) {
      ...
      if (selected)
        gPts.append(svgEl('circle', { cx: s.x, cy: s.y, r: 10, class: 'pc-selection-ring' }));
      ...
      gPts.append(marker);
      if (sk.view.showLabels)
        gPts.append(
          svgEl(
            'text',
            { x: s.x + 8, y: s.y - 8, class: `pc-label${p.macro ? ' pc-macro' : ''}` },
            p.label
          )
        );
      if (selected && !this.ctx.readonly && !this.ctx.pick)
        this.drawHandles(gPts, fr, p.id, pos, extent);
    }

    this.drawGizmo(gUi, fr);

    const pick = this.ctx.pick;
    this.hud.textContent = pick
      ? `Select ${pick.needed} point${pick.needed > 1 ? 's' : ''} (${pick.ids.length}/${pick.needed}) — Esc to cancel`
      : '';
  ...
  drawGrid(g, fr, plane, extent) {
    ...
    for (let i = -k; i <= k; i++) {
      ...
      for (const [s, e] of [
        ...
        if (seg)
          g.append(
            svgEl('line', { x1: seg[0].x, y1: seg[0].y, x2: seg[1].x, y2: seg[1].y, class: cls })
          );
      }
    }
  }
    ...
    for (const ax of AXES) {
      ...
      g.append(
        svgEl('line', {
          ...
          x2: seg[1].x,
          y2: seg[1].y,
          class: `pc-axis pc-axis-${ax.cls}`,
        })
      );
      g.append(
        svgEl(
          'text',
          { x: seg[1].x + 4, y: seg[1].y - 4, class: `pc-axis-label pc-axis-${ax.cls}` },
          ax.name
        )
      );
    }
    ...
    for (const it of items) {
      ...
      g.append(
        svgEl('line', {
          ...
          x2: ex,
          y2: ey,
          class: `pc-gizmo-axis pc-axis-${it.ax.cls}`,
          opacity: away ? 0.4 : 1,
        })
      );
      g.append(
        svgEl('circle', {
          ...
          cy: ey,
          r: 7.5,
          class: `pc-gizmo-knob pc-axis-${it.ax.cls}`,
          opacity: away ? 0.55 : 1,
      ...
      g.append(
        svgEl(
          'text',
          { x: ex, y: ey + 3.5, 'text-anchor': 'middle', class: 'pc-gizmo-text' },
          it.ax.name
        )
      );
    }
  ...
  drawDistance(g, fr, [A, B], label, cls) {
    ...
    g.append(
      svgEl('line', {
        ...
        x2: seg[1].x,
        y2: seg[1].y,
        class: `pc-glyph pc-distance ${cls}`,
      })
    );
    const mid = this.project(V.lerp(A, B, 0.5), fr);
    if (mid && label)
      g.append(
        svgEl('text', { x: mid.x + 4, y: mid.y - 4, class: `pc-glyph-label ${cls}` }, label)
      );
  }
  ...
  drawAngle(g, fr, [A, B, C], label, cls) {
    ...
    if (mid && label)
      g.append(svgEl('text', { x: mid.x, y: mid.y, class: `pc-glyph-label ${cls}` }, `${label}°`));
  }
    ...
    AXES.forEach((ax, idx) => {
      ...
      g.append(
        svgEl('line', {
          ...
          x2: seg[1].x,
          y2: seg[1].y,
          class: `pc-handle-line pc-axis-${ax.cls}`,
        })
      );
      const knob = svgEl('circle', {
        ...
        cy: e.y,
        r: 6,
        class: `pc-handle pc-axis-${ax.cls}`,
      });
  ...
  _onDown(e) {
    ...
    this._drag = {
      mode: dolly ? 'dolly' : pan ? 'pan' : 'orbit',
      lastX: sx,
      lastY: sy,
      moved: false,
    };
  ...
  _onUp(e) {
    ...
    else if ((d.mode === 'orbit' || d.mode === 'pan' || d.mode === 'dolly') && d.moved)
      this.ctx.commit('camera');
  ...
  _onKey(e) {
    ...
    switch (e.key) {
      case 'ArrowLeft':
        if (move) this.pan(-panPx, 0);
        else this.orbit(step, 0);
        break;
      case 'ArrowRight':
        if (move) this.pan(panPx, 0);
        else this.orbit(-step, 0);
        break;
      case 'ArrowUp':
        if (move) this.pan(0, -panPx);
        else this.orbit(0, step);
        break;
      case 'ArrowDown':
        if (move) this.pan(0, panPx);
        else this.orbit(0, -step);
      ...
      case '+':
      case '=':
        this.zoomAt(1 / 1.25, fr.W / 2, fr.H / 2);
        break;
      case '-':
      case '_':
        this.zoomAt(1.25, fr.W / 2, fr.H / 2);
        break;
      case 'f':
      case 'F':
      case 'Home':
