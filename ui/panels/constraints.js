...
export function createConstraintsPanel(ctx) {
  ...
  const targetSel = h(
    'select',
    { class: 'pc-in' },
    ...TARGET_OPTIONS.map(([k, label]) => h('option', { value: k }, label))
  );
  ...
  const pickBtn = h(
    'button',
    {
      ...
      onclick: () => {
        ...
        if (!kind) return;
        ctx.startPick(arityRange(kind).min, (ids) => {
          chosen = ids;
          renderPts();
        });
      },
  ...
  const submitBtn = h(
    'button',
    { class: 'pc-btn pc-primary', type: 'button', onclick: submit },
    '+ Add constraint'
  );
  const cancelBtn = h(
    'button',
    {
      ...
      type: 'button',
      hidden: true,
      title: 'Leave edit mode and go back to adding constraints',
      onclick: stopEdit,
    },
    'Cancel'
  );
  ...
  function currentKind() {
    return ctx.registry.hasConstraintKind(kindSel.value)
      ? ctx.registry.getConstraintKind(kindSel.value)
      : null;
  }
  ...
  function loadTarget(t) {
    const kind =
      t.kind === 'variable'
        ? 'value'
        : TARGET_OPTIONS.some(([k]) => k === t.kind)
          ? t.kind
          : 'value';
  ...
  function renderPts() {
    ...
    for (const prm of kind.params ?? []) {
      ...
      const input = Array.isArray(prm.values)
        ? h(
            'select',
            {
              class: 'pc-in pc-in-pt',
              title: prm.name,
              onchange: (e) => {
                paramVals[prm.name] = e.target.value;
              },
            },
            ...prm.values.map((v) => h('option', { value: v }, v))
          )
        : h('input', {
            ...
            title: prm.name,
            onchange: (e) => {
              paramVals[prm.name] = prm.type === 'number' ? Number(e.target.value) : e.target.value;
            },
          });
      input.value = cur;
    ...
    const count = Math.min(
      range.max,
      Math.max(range.min, chosen.filter(Boolean).length + (variadic ? 1 : 0))
    );
  ...
  function update() {
    ...
    for (const c of sk.constraints) {
      ...
      const kind = ctx.registry.hasConstraintKind(c.type)
        ? ctx.registry.getConstraintKind(c.type)
        : null;
      ...
      const detail = ev?.error
        ? ev.error
        : ev?.residual != null
          ? `${fmt(ev.measure)} (Δ ${fmt(ev.residual)})`
          : ev?.measure != null
            ? fmt(ev.measure)
            : '';
      ...
      list.append(
        h(
          'div',
          {
            class: `pc-item${c.enabled === false ? ' pc-disabled' : ''}${c.macro ? ' pc-macro' : ''}${selected ? ' pc-item-selected' : ''}`,
            title: ro
              ? c.id
              : selected
                ? `${c.id} — click again to stop editing`
                : `${c.id} — click to edit`,
            onclick: (e) => {
          ...
          h(
            ...
            params ? ` ${params}` : '',
            ` ${pts} `,
            h('span', { class: 'pc-target' }, targetText(c, sk, kind))
          ),
    ...
    if (!sk.constraints.length)
      list.append(h('div', { class: 'pc-muted pc-empty' }, 'No constraints yet.'));
    ...
    kindSel.replaceChildren(
      ...kinds.map((k) => h('option', { value: k.id, title: k.syntax }, k.id))
    );
