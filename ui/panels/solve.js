...
export function createSolvePanel(ctx) {
  ...
  const solveBtn = h(
    'button',
    { class: 'pc-btn pc-primary', type: 'button', onclick: () => ctx.solve() },
    'Solve'
  );
  const resetBtn = h(
    'button',
    { class: 'pc-btn', type: 'button', onclick: () => ctx.reset() },
    'Reset to seeds'
  );
  const adoptBtn = h(
    'button',
    { class: 'pc-btn', type: 'button', onclick: () => ctx.adopt() },
    'Adopt solution'
  );
  ...
  function update() {
    ...
    if (r.underConstrained)
      flags.push(
        h(
          ...
          `Under-constrained: ${r.dof.real} free degree${r.dof.real === 1 ? '' : 's'} of freedom (nearest solution to the seeds was chosen).`
        )
      );
    if (r.dof.rigid > 0) {
      flags.push(
    ...
    for (const pc of r.perConstraint) {
      ...
      list.append(
        h(
          'div',
          { class: 'pc-item', onclick: () => c && ctx.select(c.points) },
          statusDot(pc.status),
